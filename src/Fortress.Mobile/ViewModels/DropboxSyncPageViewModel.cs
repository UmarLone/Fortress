...
{
    public class DropboxSyncPageViewModel : ViewModelBase
    {
        ...
        private const string Tag = "[Dropbox]";
        ...
        private string _busyMessage = "Connecting to Dropbox…";
     ...
     string.IsNullOrEmpty(_userName) ? "D" : _userName[0].ToString().ToUpperInvariant();
     ...
     Enum.GetValues<SyncSchedule>().Select(s => new SyncScheduleOption(s)));
        ...
        // CONSTRUCTOR
        // ────────────────────────────────────────────────────────────────────
        public DropboxSyncPageViewModel(
    ...
    IBottomSheetService bottomSheetService)
             : base(navigationService)
        {
            ...
            // Restore persisted selection and mark it as selected
        ...
        private async Task RefreshStateAsync()
        {
            ...
            try
            {
                // Guard: only treat as connected if Dropbox is the stored provider
                var provider = PreferenceWrapper.Instance.CloudSyncProvider;
                var authenticated = string.Equals(provider, "Dropbox", StringComparison.OrdinalIgnoreCase)
                       && await _dropbox.IsAuthenticatedAsync();
                Log($"IsAuthenticated = {authenticated} (provider={provider})");
                if (authenticated)
                {
        ...
        {
            IsBusy = true;
            BusyMessage = "Opening Dropbox sign-in…";
            try
            {
                ...
                if (success)
                {
                    ...
                    IsConnected = true;
                    _dialogs.ShowToast("Dropbox connected successfully!");
                }
                else
                {
            ...
            catch (Exception ex)
            {
                ...
                : "Could not connect to Dropbox. Please check your internet connection and try again.";
        ...
        private async void ExecuteBackupNow()
        {
            ...
            try
            {
                ...
                if (!encResult.Succeeded)
                {
                    ...
                    NotificationType.Error, "Dropbox");
                    return;
                }

                BusyMessage = "Uploading to Dropbox…";
                var result = await _dropbox.UploadBackupAsync(Encoding.UTF8.GetBytes(encResult.Data));
                if (result.Success)
                {
                    await LoadLastSyncAsync();
                    ...
                    await _notifications.SaveAsync(
                 "Dropbox Backup Successful",
                    $"Your vault was backed up to Dropbox on {when}. {credentials.Count} logins, {authenticators.Count} 2FA codes.",
                         NotificationType.Success, "Dropbox");
                }
                else
                {
            ...
            {
                Log($"Backup: unhandled exception – {ex}");
    ...
    "Restore Vault",
         $"This will replace your current vault with the backup from Dropbox. Are you sure?",
    "Yes", "No");
            if (!confirm) return;


            IsBusy = true;
            BusyMessage = "Downloading from Dropbox…";
            try
            {
                var result = await _dropbox.DownloadBackupAsync();
                ...
                catch
                {
                    ...
                    await _notifications.SaveAsync("Dropbox Restore Failed", "Backup file is corrupted.", NotificationType.Error, "Dropbox");
                    return;
                }
                ...
                await _notifications.SaveAsync(
                     "Dropbox Restore Successful",
                $"Vault restored from Dropbox. {summary}",
                     NotificationType.Success, "Dropbox");
            }
            catch (Exception ex)
            {
        ...
        private async void ExecuteDisconnect()
        {
            ...
            var confirm = await _bottomSheetService.ConfirmAsync(
    "Disconnect Dropbox",
 $" This will remove Dropbox access from FORTRESS. Your existing backup will remain on Dropbox.",
    "Yes", "No");
            if (!confirm) return;
            ...
            try
            {
                ...
                _dialogs.ShowToast("Dropbox disconnected.");
            }
            finally { IsBusy = false; }
        }
        ...
        {
            var lastSync = await _dropbox.GetLastSyncTimeAsync();
...
// ── Helper model for the schedule picker ─────────────────────────────────
public sealed class SyncScheduleOption
{
    ...
    public SyncScheduleOption(SyncSchedule value)
    {
        ...
        {
            SyncSchedule.Manual => ("You choose when to back up", "\uE425"), // schedule
            SyncSchedule.Hourly => ("Best for frequent vault changes", "\uE192"), // timelapse
            SyncSchedule.Daily => ("Recommended – set and forget", "\uE8DF"), // today
            SyncSchedule.Weekly => ("Good balance of freshness & battery", "\uE616"), // date_range
            SyncSchedule.Monthly => ("Minimal background activity", "\uE878"), // calendar_month
            _ => ("", "\uE8DF")
        };
    }
}
