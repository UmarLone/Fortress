...
{
    public class GoogleDriveSyncPageViewModel : ViewModelBase
    {
        ...
        private const string Tag = "[GDrive]";
      ...
      System.Diagnostics.Debug.WriteLine($"{Tag} {msg}");
        ...
        private string _busyMessage = "Connecting to Google…";
        ...
        public string UserInitial =>
          string.IsNullOrEmpty(_userName) ? "G" : _userName[0].ToString().ToUpperInvariant();
       ...
       Enum.GetValues<SyncSchedule>().Select(s => new SyncScheduleOption(s)));
        ...
        public GoogleDriveSyncPageViewModel(
...
ICryptographyService crypto,
  ...
  IBottomSheetService bottomSheetService)
     : base(navigationService)
        ...
        private async Task RefreshStateAsync()
        {
            ...
            try
            {
                var authenticated = await _googleDrive.IsAuthenticatedAsync();
     ...
     _connectCommand ??= new DelegateCommand(ExecuteConnect);
        ...
        {
            IsBusy = true;
            BusyMessage = "Opening Google sign-in…";
            try
            {
                ...
                if (success)
                {
                    ...
                    IsConnected = true;
                    _dialogs.ShowToast("Google Drive connected successfully!");
                }
                else
                {
            ...
            catch (Exception ex)
            {
             ...
             : "Could not connect to Google Drive. Please check your internet connection and try again.";
            ...
            try
            {
                ...
                if (!encResult.Succeeded)
                {
                   ...
                   "Google Drive Backup Failed",
                       ...
                       NotificationType.Error, "Google Drive");
                    return;
                }

                BusyMessage = "Uploading to Google Drive…";
                var result = await _googleDrive.UploadBackupAsync(Encoding.UTF8.GetBytes(encResult.Data));
                if (result.Success)
                {
                    await LoadLastSyncAsync();
                  ...
                  "Google Drive Backup Successful",
                           $"Your vault was backed up to Google Drive on {when}. {credentials.Count} logins, {authenticators.Count} 2FA codes.",
                      NotificationType.Success, "Google Drive");
                }
                else
                {
    ...
    "Restore Vault",
 $"This will replace your current vault with the backup from Google Drive. Are you sure??",
    "Yes", "No");
            if (!confirm) return;


            IsBusy = true;
            BusyMessage = "Downloading from Google Drive…";
            try
            {
                var result = await _googleDrive.DownloadBackupAsync();
    ...
    $"Decryption failed: {decResult.ErrorMessage}",
         NotificationType.Error, "Google Drive");
                ...
                catch
                {
                    ...
                    await _notifications.SaveAsync("Google Drive Restore Failed", "Backup file is corrupted.", NotificationType.Error, "Google Drive");
                    return;
                }
    ...
    foreach (var item in snapshot.Credentials) await _storage.SaveLoginItemAsync(item);
     if (snapshot.Authenticators.Count > 0)
               ...
               $"Vault restored from Google Drive. {summary}",
             NotificationType.Success, "Google Drive");
            }
            catch (Exception ex)
            {
 ...
 $"This will remove Google Drive access from FORTRESS. Your existing backup will remain on Drive.",
    "Yes", "No");
            if (!result) return;
            ...
            try
            {
                await _googleDrive.SignOutAsync();
                ...
                _dialogs.ShowToast("Google Drive disconnected.");
            }
            finally { IsBusy = false; }
        }
      ...
      _selectScheduleCommand ??= new DelegateCommand<SyncScheduleOption>(opt =>
      {
        ...
        {
            var lastSync = await _googleDrive.GetLastSyncTimeAsync();
